...
import { fetchTasks, tasksActions, tasksReducer, TasksStateType, tasksThunks } from "features/TodolistsList/tasks.reducer";
// import { TaskPriorities, TaskStatuses } from "common/api/instance";
...
test("status of specified task should be changed", () => {
  const action = tasksThunks.updateTask.fulfilled({
    taskId: "2",
    domainModel: { status: TaskStatuses.New },   // указываем что принимает
    todolistId: "todolistId2",
  },"requestId",{
    taskId: "2",
    domainModel: { status: TaskStatuses.New },      // указываем что возвращает
    todolistId: "todolistId2",
  }
   );
...
test("title of specified task should be changed", () => {
   // можно вынести в отдельную пременную

   const par={
    taskId: "2",
    domainModel: { title: "yogurt" },   // указываем что принимает
    todolistId: "todolistId2",
   }
  const action = tasksThunks.updateTask.fulfilled(
    par, // указываем что принимает
    "requestId",
    par)// указываем что возвращает
